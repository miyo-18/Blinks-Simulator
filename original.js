...
function makeBlink(x, y) {
  ...
  var tri2 = makeTriangle(x + 15, y + 25, -pi, colors[1], 'tri2');
  var tri3 = makeTriangle(x + 15, y - 25, -pi/3, colors[2], 'tri3');
  var tri4 = makeTriangle(x - 15, y + 25, 2*pi/3, colors[3], 'tri4');
  var tri5 = makeTriangle(x - 29, y, 2*-pi/6, colors[4], 'tri5');
  var tri6 = makeTriangle(x + 29, y, pi + 2*pi/6, colors[5], 'tri6');
...
function spiral(blink) {
  blink.parts[2].render.fillStyle = blink.parts[2].label;
  // blink.parts[2].render.fillStyle = 'black';
}
